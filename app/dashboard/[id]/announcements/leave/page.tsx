...
const ImagePreview = ({ imageData, botInfo, userProfile, guildProfile, guildId, API_URL }: ImagePreviewProps) => {
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(async () => {
            ...
            try {
                // 🔴 เปลี่ยน Endpoint เป็น preview_leave_image
...
export default function LeaveSettingsPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            // 🔴 แก้ไขการดึงข้อมูล โดยเปลี่ยน endpoint เป็น leave_status
            const [me, bot, list, fonts, status] = await Promise.all([
                ...
                fetch(`${API_URL}/api/announcements/${guildId}/leave_status`, { credentials: 'include' }).then(r=>r.json())
            ]);
            
            if(!me.error) setUserProfile(me); setChannels(bot.channels || []);
            
            // 🟢 แก้ any ใน list.find
            if(Array.isArray(list)) { const g = list.find((g: GuildProfile) => g.id === guildId); if(g) setGuildProfile(g); }
    ...
    // 🟢 แก้ any โดยใช้ Type ของ EmbedData และ ImageData โดยตรง
    const handleEmbedChange = (key: keyof EmbedData, value: EmbedData[keyof EmbedData]) => setEmbedData(prev => ({ ...prev, [key]: value }));
    const handleImageChange = (key: keyof ImageData, value: ImageData[keyof ImageData]) => setImageData(prev => ({ ...prev, [key]: value }));
    ...
    // 🟢 แก้ any ใน EmbedField
    const handleUpdateField = (id: string, key: keyof EmbedField, val: string | boolean) => { setEmbedData(prev => ({ ...prev, fields: prev.fields?.map(f => f.id === id ? { ...f, [key]: val } : f) })); };
    
    const handleRemoveField = (id: string) => { setEmbedData(prev => ({ ...prev, fields: prev.fields?.filter(f => f.id !== id) })); };
    
    // 🔴 แก้ endpoint เป็น save_leave
    ...
    // 🔴 แก้ endpoint เป็น toggle_leave
    ...
    return (
        <div className="flex flex-col pb-32 p-4 md:p-8 min-h-screen max-w-[1920px] mx-auto bg-[#313338]/50 font-noto">
            ...
            <div className="flex justify-between items-center mb-6 md:mb-10 animate-page-load delay-100">
                <div><h1 className="text-2xl md:text-4xl font-extrabold text-white tracking-tight drop-shadow-md">Leave Message</h1><p className="text-[#949ba4] mt-1 md:mt-2 text-sm md:text-lg">จัดการระบบแจ้งเตือนคนออกจากเซิร์ฟเวอร์</p></div>
            ...
            <div className="relative z-[5] bg-[#2b2d31] p-5 md:p-8 rounded-2xl border border-[#1e1f22] shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 animate-page-load delay-400">
                <div className="flex items-center justify-between border-b border-gray-700/50 pb-4">
                    <h2 className="text-lg md:text-xl font-bold text-white flex items-center gap-2"><Palette className="w-5 h-5 text-[#5865f2]" /> Leave Image</h2>
                            ...
                            <div className="space-y-4 md:space-y-6">
                                <div className="bg-[#1e1f22] p-4 md:p-5 rounded-lg border border-[#3f4147] shadow-inner hover:border-[#5865f2]/50 transition-colors duration-300">
                                    ...
                                    <select 
                                        ...
                                        style={{ fontFamily: `"${imageData.font_name.replace(/\.(ttf|otf)$/i, '')}", "Kanit", sans-serif` }}
                                    >
                                        <option value="Default">Default</option>
                                        {availableFonts.map(f => {
                                            const cleanName = f.replace(/\.(ttf|otf)$/i, '');
                                            return (
                                                <option key={f} value={f} style={{ fontFamily: `"${cleanName}", "Kanit", sans-serif` }}>
                                                    {cleanName}
                                                </option>
                                            );
                                        })}
