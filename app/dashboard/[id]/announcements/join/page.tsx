...
export default function JoinSettingsPage() {
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        if (result.status === 'success') {
            Swal.fire({
                title: 'บันทึกสำเร็จ!',
                text: 'อัปเดตการตั้งค่าเรียบร้อยแล้วค่ะ',
                icon: 'success',
                background: '#2b2d31',
                color: '#dbdee1',
    ...
    return (
        <div className="max-w-2xl">
            <h1 className="text-xl font-bold text-white mb-6">ตั้งค่าข้อความต้อนรับ (Join Message)</h1>
            <form onSubmit={handleSave} className="bg-[#2b2d31] p-8 rounded-xl border border-[#1e1f22]">
                <label className="block text-[#b5bac1] font-bold mb-2">เลือกห้องสำหรับส่งข้อความ:</label>
                <select 
                ...
                <label className="block text-[#b5bac1] font-bold mb-2">ข้อความต้อนรับ:</label>
                <div className="bg-[#1e1f22] p-3 rounded-md mb-4 text-xs text-[#949ba4]">
                    💡 <b>ตัวแปรที่ใช้ได้:</b> <code>{'{user}'}</code> = แท็กชื่อคนเข้า, <code>{'{server}'}</code> = ชื่อเซิร์ฟเวอร์
                </div>
                <input 
                    type="text" 
                    value={message} 
